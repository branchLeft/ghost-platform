...
describe('requireTenantForDomain — Basic-auth parsing edge cases', () => {
  ...
  it('rejects a non-Basic auth scheme (e.g. Bearer)', async () => {
  ...
  it('rejects malformed base64 that decodes to a string with no colon separator', async () => {
  ...
  it('rejects an empty Authorization value after "Basic "', async () => {
  ...
  it('treats an empty username as fine — only the password half is used', async () => {
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue({
  ...
  it('accepts an empty password as a (failing) verification attempt rather than a parse failure', async () => {
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  it('preserves colons inside the password — only the first colon splits user from password', async () => {
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue({
      domain: 'tenant.example.com',
    });
  ...
  it('sets res.locals.tenant and calls next() on success', async () => {
    const tenant = { domain: 'tenant.example.com' };
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue(tenant);
  ...
  it('forwards a rejected verifyTenant (a store/crypto error, not a failed check) to next(err) rather than crashing or hanging', async () => {
    const dbError = new Error('database is locked');
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockRejectedValue(dbError);
    const req = fakeReq(basicAuthHeader('api', 'any-key'), 'tenant.example.com');
    const res = fakeRes();
    await middleware(req, res as unknown as Response, next);
    expect(next).toHaveBeenCalledWith(dbError);
    // Neither response branch ran — this is Express's error-handling path,
    // not a 401.
    expect(res.statusCode).toBe(0);
  });
});
...
describe('requireTenantForDomain — unknown domain vs wrong key produce the identical 401 shape', () => {
  ...
  it('unknown domain -> 401 { message: "Unauthorized" }', async () => {
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  it('known domain, wrong key -> the exact same 401 shape', async () => {
    (store.verifyTenant as ReturnType<typeof vi.fn>).mockResolvedValue(null);
...
describe('tenant isolation with a real store — the core property', () => {
  ...
  it("tenant A's own key authorises tenant A's own domain", async () => {
    const middleware = requireTenantForDomain(store);
  ...
  it("tenant B's key is likewise refused against tenant A's domain", async () => {
    const middleware = requireTenantForDomain(store);
