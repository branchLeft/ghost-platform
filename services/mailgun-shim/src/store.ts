...
export function createSqliteStore(filename = ':memory:'): ShimStore {
  ...
  // this file, or another instance of this same service racing a
  // migration below) can hold at the exact moment this one runs it.
  // Setting the timeout first means that very first statement already
  // waits rather than failing outright.
  db.exec('PRAGMA busy_timeout = 5000');

  // WAL rather than SQLite's default DELETE journal mode: the CLI
  // (register/list/events) opens this same file directly while the
