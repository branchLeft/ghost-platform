...
| `database__connection__host` **or** `database__connection__socketPath` | **Exactly one required** (MySQL) | Deploy config | **Not interchangeable — pick the one matching how this tenant reaches Cloud SQL, never both.** `host` is a TCP hostname/IP (e.g. a private-IP/VPC-connector path, not used by `infra/tenant/`'s `GhostTenant` component today). `socketPath` is a Unix domain socket path (`/cloudsql/PROJECT:REGION:INSTANCE`, what `infra/tenant/`'s Cloud Run service actually uses, paired with a `cloudSqlInstance` volume mount at `/cloudsql`). Verified directly against Ghost's source (`ghost/core/core/server/data/db/connection.js` passes `database.connection` straight into `knex()`/mysql2 unmodified; `ghost/core/core/shared/config/utils.ts`'s `sanitizeDatabaseProperties` never touches `socketPath`) and against mysql2's own connection options — mysql2 treats `host` as a TCP hostname to resolve via DNS unless it's literally `'localhost'`, so setting a Cloud SQL connection-name-shaped string as `host` fails with `ENOTFOUND` rather than opening the socket. An earlier draft of this table described `host` as "Cloud SQL host or Unix socket path", which is what produced exactly that bug in `infra/tenant/` — corrected here at source, not just in that component. |
